...
import { getJobById, createJob, updateJob } from '../../redux/slices/jobSlice';
...
const PostJob = ({ editMode, jobId }) => {
  ...
  const fetchCompanyProfileData = useCallback(async () => {
    try {
  ...
  const fetchJobDetailsData = useCallback(async () => {
    try {
      setLoading(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        const res = await dispatch(updateJob({ id: jobId, data: formData })).unwrap();
        const jobData = res.job || res;
        createdJobId = jobData._id || jobId;
        showToast.success('Job updated successfully!');
      } else {
        const res = await dispatch(createJob(formData)).unwrap();
        const jobData = res.job || res;
