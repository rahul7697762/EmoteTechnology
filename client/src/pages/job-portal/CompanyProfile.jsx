...
import { getCompanyProfile, createOrUpdateProfile, clearCompanyError } from '../../redux/slices/companySlice';
...
const CompanyProfile = () => {
  ...
  const { profile, isFetchingProfile, isUpdatingProfile, error } = useSelector((state) => state.company);
  ...
  const fetchProfile = async () => {
    try {
      const resultAction = await dispatch(getCompanyProfile()).unwrap();
      const data = resultAction.company || resultAction.data || resultAction;
      ...
      // We don't set local error state as Redux handles it
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await dispatch(createOrUpdateProfile(submitData)).unwrap();
      setSuccess(true);
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="pt-6 flex justify-end">
            <button
              ...
              className="flex items-center gap-2 px-8 py-3 bg-linear-to-r from-teal-500 to-cyan-500 text-white font-bold rounded-xl hover:shadow-lg hover:shadow-teal-500/25 transition-all disabled:opacity-50"
            >
