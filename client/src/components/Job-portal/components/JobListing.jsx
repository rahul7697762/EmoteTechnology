...
const JobListing = ({ onViewJob }) => {
  const dispatch = useDispatch();
  const { jobsList, isFetchingJobs } = useSelector((state) => state.job);
  const jobs = jobsList?.jobs || [];
  const total = jobsList?.total || 0;
  const totalPages = jobsList?.totalPages || 1;
  ...
  const fetchJobs = async () => {
    try {
      ...
      await dispatch(getAllJobs(params)).unwrap();
    } catch (error) {
      ...
      >
        <div className="text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            ...
            />
            <button className="px-6 py-3 bg-linear-to-r from-teal-500 to-cyan-500 text-white font-semibold rounded-lg hover:from-teal-600 hover:to-cyan-600 transition-all">
              Subscribe
            </button>
          </div>
        </div>
